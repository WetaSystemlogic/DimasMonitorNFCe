...
uses IniFiles, ShellAPI, pcnRetConsReciNFe,
  ...
  ACBrDANFCeFortesFr, ACBrPosPrinter, Spin, ACBrNFeDANFEFRDM, ACBrNFeDANFEFR;
...
type
  ...
  TForm1 = class(TForm)
    ...
    Label43: TLabel;
    Label44: TLabel;
    sbtnConsultaCNPJ: TSpeedButton;
    sbtnConsultaBase: TSpeedButton;
    cbRegime: TComboBox;
    Label45: TLabel;
    ...
    procedure sbtnConsultaCNPJClick(Sender: TObject);
    procedure sbtnConsultaBaseClick(Sender: TObject);
...
begin
  ...
  try
      ...
      //Gravar ID e Serie da NFCe
      Ini.WriteString( 'NFCe','Serie'         ,edtSerie.Text) ;
      Ini.WriteString( 'NFCe','ID NFCe'         ,edtIdNFCe.Text) ;
      Ini.WriteInteger('NFCe','Regime'         ,cbRegime.ItemIndex);
...
begin
  ...
  try
      ...
      //Ler ID e Serie da NFCe
      edtSerie.Text   :=  Ini.ReadString( 'NFCe','Serie'   ,'') ;
      edtIdNFCe.Text  :=  Ini.ReadString( 'NFCe','ID NFCe'   ,'') ;
      cbRegime.ItemIndex := Ini.ReadInteger('NFCe','Regime' ,0);
...
procedure TForm1.sbtnConsultaBaseClick(Sender: TObject);
begin
  //Fazer Consulta pelo Filial do Banco
  Modulo.qryFilial.Close;
  Modulo.qryFilial.SQL.Clear;
  Modulo.qryFilial.SQL.Add('select');
  Modulo.qryFilial.SQL.Add('*');
  Modulo.qryFilial.SQL.Add('from filiais');
  Modulo.qryFilial.Open();

  //Chamar o Form de Selecao de Filial
  frmConsultaFilial :=  TfrmConsultaFilial.Create(Self);
  frmConsultaFilial.ShowModal;
end;

procedure TForm1.sbtnConsultaCNPJClick(Sender: TObject);
begin
  //Chamar Form de Consulta de CNPJ
...
begin
  ...
  //Tentativas de Envio
  for I := 0 to StrToInt(edtTentativas.Text) do
  begin
    try
      ACBrNFe1.Enviar(vNumLote,True);
    except on E:Exception do
      ShowMessage('Não foi possivel estabelecer conexao'+#13
      +'com o servidor da Sefaz.'+#13+E.Message);
    end;
  end;
...
var
  ThreadStatus: Status;
begin
  if frmPrincipal.TrayIcon.Visible = False then begin
    ...
    if frmPrincipal.rgMensagemRetorno.ItemIndex = 0 then begin    
      ThreadStatus  :=  Status.Create(False);
      ThreadStatus.Resume;  
    end;
  end;
...
//Para NFe Avulsa preencha os campos abaixo
{
  ...
  NotaF.NFe.Dest.CNPJCPF           := '05697633394';
  NotaF.NFe.Dest.IE                := '';
  NotaF.NFe.Dest.ISUF              := '';
  NotaF.NFe.Dest.xNome             := 'WESSEL NIEMYER CARVALHO MARINHO';

  NotaF.NFe.Dest.EnderDest.Fone    := '98982604909';
  NotaF.NFe.Dest.EnderDest.CEP     := 65055845;
  NotaF.NFe.Dest.EnderDest.xLgr    := 'Rua Epitacio Cafeteira';
  NotaF.NFe.Dest.EnderDest.nro     := '90';
  NotaF.NFe.Dest.EnderDest.xCpl    := '';
  NotaF.NFe.Dest.EnderDest.xBairro := 'Vila Brasil';
  NotaF.NFe.Dest.EnderDest.cMun    := 2111300;
  NotaF.NFe.Dest.EnderDest.xMun    := 'São Luis';
  NotaF.NFe.Dest.EnderDest.UF      := 'MA';
...
//Campos específicos para venda de combustível(distribuidoras)
{
  ...
  Produto.Imposto.ICMS.CSOSN   := csosn400; //Imposto para Empresa do Regime Simples Nacional
